	...
	foreach ($arr as $key => $val) {
		...
		if (0 < intval($GLOBALS['_CFG']['use_storage']) && $goods['extension_code'] != 'package_buy' && $store_id == 0 && $row['cloud_id'] == 0) {
			...
			if (0 < intval($GLOBALS['_CFG']['use_storage']) && 0 < $store_id && $row['cloud_id'] == 0) {
				...
				$products = get_warehouse_id_attr_number($row['goods_id'], $row['goods_attr_id'], $goodsInfo['ru_id'], 0, 0, 0, '', $store_id);
...
', '', $_CFG['invoice_content']));
	...
	if (0 < $store_id) {
		$cart_goods_list = cart_goods($flow_type, $_SESSION['cart_value'], 1, $region_id, $area_id, $area_city, '', $store_id);
		$cart_goods_list_new = cart_by_favourable($cart_goods_list);
		$cart_goods = get_new_group_cart_goods($cart_goods_list_new);
				...
				if (0 < $stages_total && $is_stages == 1) {
					for ($i = 1; $i <= $stages_total; $i++) {
						...
						if (!$GLOBALS['db']->getOne($sql, true)) {
							$pay_log_other = array('log_id' => $bt_log_id, 'baitiao_id' => $bt_info['baitiao_id'], 'stages_num' => $i, 'stages_price' => $stages_one_price, 'add_time' => $order_time);
...
');
		...
		$solve_order = array('user_id' => $user_id, 'orec_id' => $done_cart_value, 'flow_type' => $flow_type, 'solve_type' => DEALCONCURRENT_PAY_ORDER, 'add_time' => $order_time);
...
');
	...
	$db->query($sql);
	$order_info = get_table_date('order_info', ' order_sn = \'' . $order_sn . '\' ', array('order_id'));
