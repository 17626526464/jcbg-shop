...
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER, $order_type = 0, $deposit_fee = 0)
{
	...
	if ($change_desc && $order_type) {
		$change_desc_arr = $change_desc ? explode(' ', $change_desc) : array();
...
{
	$sql = 'SELECT cat_id, cat_name FROM ' . $GLOBALS['ecs']->table('merchants_category') . (' WHERE parent_id = \'' . $cat_id . '\' AND user_id = \'' . $ru_id . '\' ORDER BY sort_order, cat_id DESC');
	$res = $GLOBALS['db']->getAll($sql);
	return $res;
}
	...
	if ($store_list) {
		...
		foreach ($store_list as $key => $row) {
			...
			if (is_spec($is_spec) == true) {
				$products = get_warehouse_id_attr_number($row['goods_id'], $spec_arr, $row['ru_id'], 0, 0, 0, '', $row['id']);
