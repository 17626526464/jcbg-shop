...
function sec_goods_stats($sec_id, $begin_time = '', $end_time = '')
{
	...
	$sql = 'SELECT COUNT(*) AS total_order, SUM(g.goods_number) AS total_goods ' . 'FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o, ' . $GLOBALS['ecs']->table('order_goods') . ' AS g ' . ' WHERE o.order_id = g.order_id ' . $where . ' AND g.extension_code = \'seckill' . $sec_id . '\' ' . (' AND g.goods_id = \'' . $sec_goods_id . '\' ') . ' AND (order_status IN (\'' . OS_UNCONFIRMED . '\',\'' . OS_CONFIRMED . '\',\'' . OS_SPLITED . '\',\'' . OS_SPLITING_PART . '\'))';
	$stat = $GLOBALS['db']->getRow($sql);
