	...
	foreach ($row as $key => $value) {
		...
		if (empty($value['chargeoff_status'])) {
			...
			if ($value['chargeoff_status'] == 1) {
				$where_order = ' AND ' . db_create_in(array(OS_UNCONFIRMED), 'sbo.shipping_status') . ' AND ' . db_create_in(array(PS_UNPAYED), 'sbo.pay_status') . ' AND ' . db_create_in(array(OS_RETURNED), 'sbo.order_status');
				$sql = 'SELECT SUM(sbo.return_amount) AS return_amount, SUM(sbo.return_shippingfee) AS return_shippingfee FROM ' . $GLOBALS['ecs']->table('seller_bill_order') . ' AS sbo ' . ' WHERE sbo.bill_id = \'' . $value['id'] . '\'' . $where_order . ' LIMIT 1';
	...
	if ($mintime && $maxtime) {
		...
		if ($min_year < $max_year) {
			...
			if (0 < $min_count) {
				...
				$last_year_end = $min_year . '-12-31 23:59:59';
				$day_start = local_strtotime($last_year_start);
				$day_end = local_strtotime($last_year_end);
				$bill_id = get_bill_id($seller_id, $cycle, $day_start, $day_end);
				if (!$bill_id && ($mintime <= $day_start && $day_end <= $maxtime)) {
