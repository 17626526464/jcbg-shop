...
');
	...
	$where .= ' AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 WHERE oi2.main_order_id = o.order_id) = 0 ' . $where_common;
	...
	if (admin_priv('order_view', '', false)) {
		...
		$arr['await_ship'] = $db->getOne('SELECT count(*) FROM ' . $ecs->table('order_info') . ' AS oi ' . ' WHERE 1 AND (oi.shipping_status = \'' . SS_UNSHIPPED . '\' OR oi.shipping_status = \'' . SS_PREPARING . '\') AND oi.pay_status = \'' . PS_PAYED . '\' AND oi.order_status = \'' . OS_CONFIRMED . '\' AND (SELECT ore.ret_id FROM ' . $GLOBALS['ecs']->table('order_return') . ' AS ore WHERE ore.order_id = oi.order_id LIMIT 1) IS NULL ' . $where_og);
	}

	if (admin_priv('order_back_apply', '', false)) {
		$sql = 'SELECT count(ore.ret_id) FROM ' . $GLOBALS['ecs']->table('order_return') . ' AS ore LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . (' AS o ON o.order_id = ore.order_id WHERE o.order_status NOT IN (2,3,4,7,8) ' . $where_common . ' ');
