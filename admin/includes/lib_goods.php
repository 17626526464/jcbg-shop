...
function goods_list($is_delete = 0, $real_goods = 1, $conditions = '', $review_status = 0, $real_division = 0)
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . (' WHERE ' . $where . ' AND g.is_delete = \'' . $is_delete . '\'');
		...
		$sql = 'SELECT g.goods_id,g.goods_thumb, g.goods_name, g.user_id, g.brand_id, g.goods_type, g.goods_sn, g.shop_price,g.sales_volume, g.sales_volume_base, ' . 'g.is_on_sale, g.is_best, g.is_new, g.is_hot, g.sort_order, g.goods_number, g.integral, g.commission_rate, ' . 'g.is_promote, g.model_price, g.model_inventory, g.model_attr, g.review_status, g.review_content, g.store_best, ' . 'g.store_new , g.store_hot , g.is_real, g.is_shipping, g.stages, g.goods_thumb, ' . 'g.is_alone_sale, g.is_xiangou, g.promote_end_date, g.xiangou_end_date, g.bar_code, g.freight, g.tid ' . $select . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . (' WHERE  ' . $where . ' AND g.is_delete = \'' . $is_delete . '\' ') . (' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ') . ' LIMIT ' . $filter['start'] . (',' . $filter['page_size']);
