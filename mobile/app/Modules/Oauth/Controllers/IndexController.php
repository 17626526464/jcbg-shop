...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
	...
	public function actionBindRegister()
	{
		if (IS_POST) {
			...
			if (!empty($userinfo)) {
				...
				$user_connect = dao('users')->alias('u')->join(C('DB_PREFIX') . 'connect_user cu on u.user_id = cu.user_id')->field('u.user_id, u.user_name, u.mobile_phone')->where($final)->find();
	...
	protected function doRegister($res, $type = '')
	{
		...
		if (is_dir(APP_DRP_PATH) && 0 < $res['drp_parent_id']) {
