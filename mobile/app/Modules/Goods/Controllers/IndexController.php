...
');
		...
		if ($sec_goods_id) {
			$this->redirect(url('seckill/index/detail', array('id' => $sec_goods_id, 'tmr' => 0)));
			exit();
		}
...
');
	...
	public function get_is_seckill($goods_id = 0)
	{
		...
		$sql = 'SELECT sg.id AS sec_goods_id FROM ' . $GLOBALS['ecs']->table('seckill_goods') . ' AS sg ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('seckill_time_bucket') . ' AS stb ON sg.tb_id = stb.id ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('seckill') . ' AS s ON s.sec_id = sg.sec_id ' . (' WHERE goods_id = \'' . $goods_id . '\' AND s.begin_time <= \'' . $date_begin . '\' AND s.acti_time > \'' . $date_begin . '\' AND s.is_putaway = 1 ') . (' AND s.review_status = 3 AND stb.begin_time <= \'' . $stb_time . '\' AND stb.end_time > \'' . $stb_time . '\' ORDER BY stb.begin_time ASC');
