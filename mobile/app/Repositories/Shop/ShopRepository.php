...
class ShopRepository
{
	...
	{
		$time = local_gettime();
		$ads = \App\Models\TouchAd::select('ad_id', 'touch_ad.position_id', 'media_type', 'ad_link', 'ad_code', 'ad_name')->with(array('position'))->join('touch_ad_position', 'touch_ad_position.position_id', '=', 'touch_ad.position_id')->where('start_time', '<=', $time)->where('touch_ad_position.tc_type', 'banner')->where('touch_ad_position.ad_type', $tc_type)->where('end_time', '>=', $time)->where('enabled', 1);
	...
	{
		$res = \App\Models\ArticleXiao::select('title', 'content', 'article_id')->where('is_open', 1)->where('cat_id', '>', 0)->limit(10)->orderBy('add_time', 'desc')->get()->toArray();
		return $res;
	}
}
