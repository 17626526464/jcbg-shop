	...
	foreach ($newInfo as $key => $info) {
		...
		do {
			...
			if (C('shop.sms_order_placed') == '1' && $sms_shop_mobile != '') {
				if (!empty($auto_sms)) {
					$autoData = array('item_type' => 1, 'user_id' => $row['user_id'], 'ru_id' => $key, 'order_id' => $new_orderId, 'add_time' => gmtime());
					dao('auto_sms')->data($autoData)->add();
				}
				else {
					$msg = array('consignee' => $row['consignee'], 'order_mobile' => $row['mobile'], 'ordermobile' => $row['mobile']);
			...
			if (C('shop.send_service_email') && $service_email != '') {
				if (!empty($auto_sms)) {
					$autoData = array('item_type' => 2, 'user_id' => $row['user_id'], 'ru_id' => $key, 'order_id' => $new_orderId, 'add_time' => gmtime());
					dao('auto_sms')->data($autoData)->add();
				}
				else {
					$cart_goods = $arr[$key];
			...
			if ($virtual_goods && $flow_type != CART_GROUP_BUY_GOODS) {
				...
				if (virtual_goods_ship($virtual_goods, $msg, $order_sn, true)) {
					...
					if ($GLOBALS['db']->getOne($sql) <= 0) {
						update_order($arr[$key][$j]['order_id'], array('order_status' => OS_CONFIRMED, 'shipping_status' => SS_SHIPPED, 'shipping_time' => gmtime()));
					}
				}
			}
		}
