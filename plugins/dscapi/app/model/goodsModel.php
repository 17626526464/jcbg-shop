...
abstract class goodsModel extends \app\func\common
{
	...
	public function get_where($val = array(), $alias = '')
	{
		...
		$where .= \app\func\base::get_where($val['province_name'], 'r1.province_name');
		$where .= \app\func\base::get_where($val['city_name'], 'r1.city_name');
	...
	public function get_select_list($table, $select, $where, $page_size, $page, $sort_by, $sort_order)
	{
		...
		if ($table == 'warehouse_area_goods') {
			...
			else if (strpos($where, 'city_name') !== false) {
			...
			$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('warehouse_area_goods') . ' AS wag ' . $left_join . ' WHERE ' . $where;
		...
		if ($table == 'warehouse_area_goods') {
			...
			$sql = 'SELECT wag.*, r1.region_name FROM ' . $GLOBALS['ecs']->table('warehouse_area_goods') . ' AS wag ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('region_warehouse') . ' AS r1 ON r1.region_id = wag.region_id ' . $region_where . ' WHERE ' . $where;
	...
	public function get_select_info($table, $select, $where)
	{
		...
		if ($table == 'warehouse_area_goods') {
			...
			$sql = 'SELECT wag.*, r1.region_name FROM ' . $GLOBALS['ecs']->table('warehouse_area_goods') . ' AS wag ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('region_warehouse') . ' AS r1 ON r1.region_id = wag.region_id ' . $region_where . ' WHERE ' . $where . ' LIMIT 1';
